...
### 1.4 프로젝트 일정
- **총 소요 기간**: 3주 (Day 1 ~ Day 15)
- **Phase 1 (DB)**: 1일 (2026-02-11)
- **Phase 2 (BE)**: 5일 (2026-02-12 ~ 2026-02-16)
- **Phase 3 (FE)**: 5일 (2026-02-17 ~ 2026-02-21)
- **Phase 4 (TEST)**: 3일 (2026-02-22 ~ 2026-02-24)
- **Phase 5 (배포 준비)**: 2일 (2026-02-25 ~ 2026-02-26)
...
#### 완료 조건 체크리스트
- [ ] PostgreSQL 17 설치 완료 (Windows/Mac/Linux 또는 Docker)
- [ ] 포트 5432 정상 작동 확인
- [ ] 초기 접속 테스트 성공 (`psql -U postgres`)
- [ ] postgres 기본 사용자 암호 설정 완료
...
- [ ] `todoapp` 애플리케이션 사용자 생성 완료
- [ ] todoapp 사용자에 권한 부여 완료
- [ ] 접속 테스트 성공 (`psql -U todoapp -d my_todolist`)
...
#### 완료 조건 체크리스트
- [ ] 샘플 사용자 2명 생성 완료
- [ ] 각 사용자별 샘플 할일 3개씩 생성 완료
- [ ] 다양한 마감일 포함 (과거, 현재, 미래)
- [ ] 일부 할일의 완료 상태 설정 (done=true)
- [ ] 데이터 정합성 검증 완료
...
|------|------|-------|------|---------|--------|
| 2/11 | 화 | DB | DB-1 ~ DB-4 | 2.5시간 | DB 관리자 |
| 2/12 | 수 | BE | BE-1 ~ BE-5 | 8시간 | 백엔드 개발자 |
| 2/13 | 목 | BE | BE-6 ~ BE-10 | 7.5시간 | 백엔드 개발자 |
| 2/14 | 금 | FE | FE-1 ~ FE-3 | 4.5시간 | 프론트엔드 개발자 |
| 2/17 | 월 | FE | FE-4 ~ FE-7 | 9시간 | 프론트엔드 개발자 |
| 2/18 | 화 | FE | FE-7 (계속), 통합 준비 | 4시간 | 프론트엔드 개발자 |
| 2/19 | 수 | TEST | TEST-1 ~ TEST-3 | 5시간 | QA + 보안 담당자 |
| 2/20 | 목 | TEST | TEST-4 ~ TEST-5 | 3시간 | QA + 문서 관리자 |
| 2/25 | 화 | DEPLOY | DEPLOY-1 | 2시간 | DevOps |
...
| **Phase 2 완료** | 2/13 | 백엔드 API 완료 | 모든 엔드포인트 작동, 테스트 통과 |
| **Phase 3 완료** | 2/18 | 프론트엔드 완료 | 모든 UI 완성, 통합 테스트 준비 |
| **Phase 4 완료** | 2/20 | 전체 테스트 완료 | 모든 테스트 케이스 통과 |
| **프로젝트 완료** | 2/26 | 배포 준비 완료 | 프로덕션 배포 가능 상태 |
